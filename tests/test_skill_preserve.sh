...
# Prefer merge-base with main when available
base_ref=""
...
if ! git cat-file -e "${base_ref}:skills/agent-letterbox/SKILL.md" 2>/dev/null; then
  echo "SKIP: no base SKILL.md at $base_ref (new skill allowed only if intentional)"
  # Still require frontmatter keys on current file
else
  base_tmp="$(mktemp)"
  git show "${base_ref}:skills/agent-letterbox/SKILL.md" >"$base_tmp"
  base_lines="$(wc -l < "$base_tmp" | tr -d ' ')"
  head_lines="$(wc -l < "$skill" | tr -d ' ')"
  # Fail on large deletions: head must not shrink by >25% or >30 lines
  min_keep=$(( base_lines * 75 / 100 ))
  if (( head_lines < min_keep )) || (( base_lines - head_lines > 30 )); then
  ...
  # Frontmatter keys that must survive
...
# Current file must have YAML frontmatter fence + required keys
